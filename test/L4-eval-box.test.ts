...
describe('HW3 bound', () => {
    ...
    // it("prints the trace of (square 2)", () => {
    ...
    it("doesn't print the trace of untraced proc", () => {
        ...
        (L4
            ...
            (arithsum 4)
        )`), evalProgram)).toEqual(makeOk(10));
        // expect(consoleSpy).toHaveBeenLastCalledWith("< 24");
    });

    // it("doesn't print the trace of procs internal to traced proc", () => {
    //     const consoleSpy = jest.spyOn(console, 'log');
    //     expect(bind(parseL4(`
    //     (L4
    //         (define facti
    //             (letrec
    //                 ((aux (lambda (n acc) (if (= n 0) acc (aux (- n 1) (* n acc))))))
    //                 (lambda (n) (aux n 1))
    //             )
    //         )
    ...
    //     expect(bind(parseL4("(L4 (define x 5) (trace x) (+ x x))"), evalProgram)).toSatisfy(isFailure);
    // })

    // it("shouldn't trace lists", () => {
    //     expect(bind(parseL4("(L4 (define x '(1 2 3)) (trace x) (+ x x))"), evalProgram)).toSatisfy(isFailure);
    // })

    // it("shouldn't trace booleans", () => {
    //     expect(bind(parseL4("(L4 (define x #t) (trace x) (+ x x))"), evalProgram)).toSatisfy(isFailure);
    // })
