...
export const makeTimeExp = (exp: CExp): TimeExp =>
    ({tag: "TimeExp", exp: exp});
...
export const parseL4SpecialForm = (op: SpecialFormKeyword, params: Sexp[]): Result<BoundExp> | Result<NumExp | BoolExp | StrExp | PrimOp | VarRef | AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | BoundExp | TimeExp> =>
...
const parseBoundExp = (params: Sexp[]): Result<BoundExp> =>
    isNonEmptyList<Sexp>(params) ?
        (params.length === 0) ? makeFailure(`bound? missing 1 argument: ${format(params)}`) :
            (params.length > 1) ? makeFailure(`bound? has too many arguments: format(params)`) :
                parseGoodBoundExp(first(params)):
        makeFailure("bound? is missing 1 arg");
...
const parseTimeExp = (params: Sexp[]): Result<TimeExp> =>
    params.length !== 1 ? makeFailure(`Expression not of the form (time <CExp>): ${format(params)}`) :
        mapv(mapResult(parseL4CExp, params), (cexps: CExp[]) => makeTimeExp(cexps[0]));
